...
<div class="container mx-auto px-4 py-8 animate-fade-in-down">
    <h1 class="text-4xl font-bold mb-8 text-indigo-600">Create New Task</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('tasks.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 @error('title') border-red-500 @enderror" id="title" name="title" required maxlength="255" value="{{ old('title') }}">
                @error('title')
            ...
            <div class="mb-6">
                ...
                <textarea class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror" id="description" name="description" required rows="4">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
